...
public class Main {
    public static void main(String[] args) {
        ...
        apple = keyborad.nextInt();
        System.out.printf("배(%d 원) 개수 입력 : ", PEAR);
        pear = keyborad.nextInt();
        System.out.print("지불 금액 입력 : ");
        pay = keyborad.nextInt();

        price1 = apple * APPLE;
        price2 = pear * PEAR;
        result = pay >=price1+price2 ? String.format("거스름 돈 : %d원",pay - (price1+price2));
                 String.format("지불한 금액이 %,d원이 부족합니다.",price1+price2 -pay);

        System.out.printf("사과 %개의 가격 :%,d원\n", apple, price1);
        System.out.printf("배 %개의 가격 : %,d원\n",pear, price2);
