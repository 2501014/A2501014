...
public class Main {
    public static void main(String[] args) {
        Scanner ketborad = new Scanner(System.in);
        ...
        while (i < movie.length){
            ...
            movie[i] = ketborad.nextFloat();

            if(movie[i] >= MIN && movie[i] <= MAX)
                i++;
            else {
                System.err.printf("ERROR : %.1f ~ %1.f", MIN, MAX);
                System.in.read();
            }
        }
        ...
        System.out.printf("평균점수 : %3.1 점\n", avg);
        System.out.printf("만점자(%.0f점) 수 : %d 명\n", MAX, maxCount);
